...
log_file = CACHE_DIR / Path(sys.argv[0]).with_suffix(".log").name
...
def demo_init() -> None:
    ...
    with timer() as t:
        ...
        GENERATOR_OBJ = RxImageGenerator(
            ...
            shapes="ROUND",
            colors="WHITE",
            fraction_pills_type=(0.9, 0.1),
            apply_composed_aug=False,
            apply_bg_aug=False,
        )
...
def demo_load_image(image: np.ndarray, ref_image: np.ndarray, MSG: str) -> None:
    ...
    """
    assert isinstance(IMAGE_OBJ, RxVision)
...
def demo_verify() -> list[tuple[np.ndarray, str]]:
    ...
    with timer() as t:
        verify_scores = []
        for i, score in enumerate(IMAGE_OBJ.similarity_scores):
            img = IMAGE_OBJ.ROIs[i].copy()
            if float(score) < 0.1:  # TODO: Hardcoded threshold that needs to be replaced.
                cv2.rectangle(img, (0, 0), img.shape[1::-1], (255, 0, 0), 5)
            else:
                cv2.rectangle(img, (0, 0), img.shape[1::-1], (0, 255, 0), 5)
            verify_scores.append((img, f"{score:.3f}"))
...
def demo_video_counter(dosage_str: str, frame: np.ndarray) -> Tuple[np.ndarray, str]:
    ...
    assert isinstance(STREAM_OBJ, RxVision)
...
def create_demo(functions: Sequence[str]) -> gr.Blocks:
    """
    ...
    with gr.Blocks(css="footer {visibility: hidden}", theme=gr.themes.Soft()) as demo:
        ...
        if "Pipeline" in set(functions):
            ...
            with gr.Tab("Segmentation"):
                with gr.Row():
                    gallery_mROI = gr.Gallery(
                        label="Masked ROIs", columns=5, object_fit="scale-down", height=480
                    )
