...
class RxDetection(RxBase):
    ...
    def _load_model(self) -> None:
        ...
        if self._model_path.startswith(SHARED_REMOTE_DIR):
            self.model_path = fetch_from_remote(self._model_path, cache_dir=".cache/counting")
        logger.assertion(Path(self.model_path).exists(), f"Model path {self._model_path} does not exist.")

        # Load the model.
        self._model = yolo_model.get(
            self._yolo_model, num_classes=self._n_classes, checkpoint_path=self.model_path
        )
        self._model.eval()
...
if __name__ == "__main__":
    from rx_connect.types.image import RxImageBase, RxImageCount

    img_path = "./data/synthetic/detection/images/0_20.jpg"
    cfg_path = f"{TYPES_DIR}/configs/Dev/counter_config.yml"

    # Test the class
    image_obj = RxImageBase()
    image_obj.load_from_path(img_path)
    count_obj = RxImageCount(image_obj)

    pill_count_obj = RxDetection(cfg_path)
    count_obj.set_counter(pill_count_obj)

    logger.info(f"Number of pills: {count_obj.get_count()}.")
